...
function InputAuth({
  ...
  return (
    <div className={style.inputAuth} {...props}>
      ...
      <p className={(error && value !== '' ) ?  style.inputAuth__text_error : style.inputAuth__text }>{title ? title : 'введите наименование поля'}</p>
      <input required={required} className={!error ? (value !== '' ? style.inputAuth__input : style.inputAuth__input_padding) :  (value !== '' ? style.inputAuth__input_error : style.inputAuth__input_padding)}
             type={password && "password"} id={id}
             name={name} placeholder={placeholder} value={value} onChange={onChange ? onChange : null} disabled={disabled}/>
